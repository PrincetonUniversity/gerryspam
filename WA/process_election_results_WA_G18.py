...
raw_elec = '/Volumes/GoogleDrive/Shared drives/princeton_gerrymandering_project/OpenPrecincts/States for site/Washington/Election Results/Raw/20181106_AllStatePrecincts.csv'

elec_df = pd.read_csv(raw_elec)
elec_df['loc_prec']=elec_df['CountyCode'].map(str) + ',' + elec_df['PrecinctName'].map(str) + ',' + elec_df['PrecinctCode'].map(str) 
...
prec_elec_rn = prec_elec.rename(columns = {
        'Votes U.S. Senator Maria Cantwell' : 'G18DSEN',
        'Votes U.S. Senator Susan Hutchison' : 'G18RSEN'})

#get rid of other columns and save
#this is ready to be matched to precinct names now
prec_elec_rn = prec_elec_rn[['G18DSEN','G18RSEN']]
...
prec_elec_rn.to_csv('/Volumes/GoogleDrive/Shared drives/princeton_gerrymandering_project/OpenPrecincts/States for site/Washington/Election Results/WA_G18_SOS_SEN.csv')
